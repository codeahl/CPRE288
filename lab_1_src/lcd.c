...
//Rotates string around first level of lcd1
void lcd_rotatingBanner(char data[]) {
    int count = 0;
       int i;
       char displayStr[21]; // LCD has 20 chars + null terminator
       int len = strlen(data);
       ...
       while (1) {
           ...
           // Fill the display string with the chars
           if (count < 20) {
               for (i = 0; i <= count; i++) {
                   displayStr[19 - count + i] = data[i];
               }

           } else {
               for (i = 0; i < 20 && (count - 20 + i) < len; i++) {
                   displayStr[i] = data[count - 20 + i];
               }
           }

           // Print the simulated LCD output
           lcd_printf("%s", displayStr);
           fflush(stdout);

           // Delay for 300ms
           timer_waitMillis(300);

           // Reset count after full cycle
           if (count >= (20 + len)) {
               count = 0;
               timer_waitMillis(300);
           } else {
...
//Rotates string around first level of lcd2
//void lcd_rotatingBanner(char str[]) {
//
//        char strCopy[22] = ""; // Allocate enough space
//
//        int count = 0;
//        int count2 = 0;
//        int x = 0;
...
//        while (1)
//        { // Infinite loop
//            timer_waitMillis(300); // Wait for 0.3 second
//
//            if (count < 20)
//            { // Increase count up to 20
//                strCopy[count] = str[count]; // Copy one letter at a time
//                strCopy[count + 1] = '\0';   // Ensure null termination
//
//                lcd_printf("%s", strCopy); // Print updated string
//
//                count++;
//                count2++;
//            }
//            else if (count == 20)
//            { // Reset to 0 after 20 characters
//
//                if (str[count2] != '\0')
//                {
//                    for (i = 0; i < count - 1; i++)
//                    {
//                        strCopy[i] = strCopy[i + 1]; // Shift left
//                    }
//                    strCopy[count - 1] = str[count2]; // Add new char
//                    strCopy[count] = '\0'; // Ensure null termination
//                    count2++;
//
//                    lcd_printf("%s", strCopy);
//                }
//                else
//                {
//                    if (x <= 20) // Add spaces
...
//                        }
//                        strCopy[count - 1] = ' '; // Correct space assignment
//                        strCopy[count] = '\0';
//                        lcd_printf("%s", strCopy);
//                        x++;
//                    }
//                    if (x == 21) {
//                        count = 0; // Restart animation
//                        count2 = 0;
//                        x = 0;
//                    }
//                }
